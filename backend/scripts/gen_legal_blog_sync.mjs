...
// Syncs ONLY the legal category (6 articles x 8 languages = 48 translations)
// from local dev DB to prod. Upsert by slug, not id — dev/prod generate their
// own gen_random_uuid() independently at INSERT time, so ids never line up
// even for the "same" logical row. Other categories (deposit-safety/
// bike-models/routes/digital-nomads) and any other articles are intentionally
// out of scope — not touched by this script. Pattern matches
// backend/scripts/gen_blog_sync.mjs (deposit-safety sync).
...
async function main() {
  ...
  if (articles.length !== 6) throw new Error(`expected 6 legal articles, found ${articles.length}`);
  ...
  out.push("-- Syncs ONLY article_categories.slug = 'legal' (6 articles x 8 languages = 48 translations)");
  out.push('-- from local dev DB to prod. Idempotent (INSERT ... ON CONFLICT DO UPDATE), safe to re-run.');
  out.push('-- Does NOT touch deposit-safety/bike-models/routes/digital-nomads categories.');
  ...
  for (const a of articles) {
    out.push(
      ...
      `  FROM article_categories ac WHERE ac.slug = 'legal'\n` +
  ...
  out.push(`-- article_translations: ${translations.length} rows (6 articles x 8 languages)`);
  ...
  await writeFile(new URL('./legal_blog_sync.sql', import.meta.url), out.join('\n') + '\n');
  console.log('written legal_blog_sync.sql —', articles.length, 'articles,', translations.length, 'translations');
  await pool.end();
}
