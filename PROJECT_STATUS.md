...
выполнена** — ждёт подтверждения Дмитрия, что код запушен и Coolify
редеплой прошёл (иначе на проде до выхода кода будет битая картинка
вместо видео, тот же урок, что и с `049_article_display_order.sql`
26.08.2026). Бэкап (`pg_dump --data-only --table=article_translations`)
снят заранее.

**Push + редеплой фронта подтверждены Дмитрием тем же днём** — `origin/main`
на `8b74b15` (код `8022b2d` + 2 docs-коммита поверх). Бэкенд в этом
диапазоне не менялся, редеплой бэкенда не требовался.
...
прод обратно. Видео-вставка Versys (см. выше) при этом остаётся
невыполненной отдельно — два независимых куска работы в одной сессии.
