...
_Обновлено: 2026-07-29 (сессия: R2-ресинк фото после пересборки прошлой
...
_Предыдущее обновление: 2026-07-24 (чанк **Deploy в процессе**: backend+frontend на Contabo
VPS + Coolify 4.1.2, оба `running:healthy`, миграции 36/36 по коду — не
гарантировано без свежего `npm run migrate:status`, см. раздел «Деплой —
В ПРОЦЕССЕ» ниже; сидинг
CRM с dev-БД — ЗАВЕРШЁН, фото на R2 — ЗАВЕРШЁНО. **HTTPS на временных
sslip.io-доменах ЗАРАБОТАЛ** (найден и исправлен баг: домены в Coolify были
сохранены как `http://`, HTTPS-роутер/сертификат не создавались вообще —
подробности в чанке ниже; сертификат подтверждён напрямую на сервере, не
по статусу Coolify). DNS cutover на боевой домен — отдельная, всё ещё не
начатая задача. Дальше — 5 блоков доработок каталога, все ЗАВЕРШЕНЫ и
...
replacement groups, общая навигация на 5 разделов). Дальше — чанк
**Визуальный редизайн главной + фикс фильтров ЗАВЕРШЁН**: двухуровневый
фильтр каталога (Все/Скутеры/Мотоциклы + модели, без скролла), hero-фон
с реальным фото, полоска доверия, «Как это работает», витрина 6 моделей,
отзывы (плейсхолдер-данные, явно помечены). Ранее — чанк **SEO
...
- **Деплой — В ПРОЦЕССЕ** (чанк Deploy, подробности — раздел выше): провайдер
  ...
  и на **HTTPS** (временные sslip.io-домены, реальный Let's Encrypt сертификат
  подтверждён напрямую на сервере), оба на актуальном коммите. DNS cutover на
  боевой домен — отдельно, не начато. Миграции 36/36 по коду — но этой
