...
_Обновлено: 2026-08-01 (сессия: **email/DNS-гигиена + Merchant Listing SEO-фиксы +
Family-контент + переводы + разграничение блоков на карточке товара** — шесть
коммитов, `b6a884c`…`5e3fbc0`. Контактный email сайта переключён на
`rentbalibike@gmail.com` (домен bikebalirent.com почтовые ящики никогда не
использовал — подтверждено Дмитрием); следом DNS домена задекларирован как
non-mail: null MX + `SPF: -all` + `DMARC: p=reject`, снесены 5 неиспользуемых
Hostinger-записей (autoconfig/autodiscover/3×DKIM). Три пункта из Google Search
Console «Данные о товарах продавца» закрыты честно на реальных данных:
`category` → фиксированное значение Google Product Taxonomy (ID 919,
невидимо, каталожный фильтр-ярлык на странице не тронут),
`hasMerchantReturnPolicy: MerchantReturnNotPermitted` (обосновано пунктом
самого договора аренды о невозврате оплаты), `shippingDetails` — 3 тарифа из
живого `/api/delivery-fee-rules`. Новая публичная `/terms` (текст реального
договора аренды, тихая ссылка в футере, не в главной навигации) переведена на
все 8 языков сайта — в каждом non-en варианте сноска, что при расхождении
трактовки действует английский оригинал. Добавлен новый Family-level блок
"Key Benefits / Expert Tips / FAQ" (19 моделей, не 79 цветов — новая таблица
`family_content_translations`) под калькулятором, с явным визуальным
разграничением (заголовок + карточка), плюс короткое `description` заполнено
для всех 101 активных продуктов тем же intro-текстом. Попутно дополнены
`family_specs` для 4 моделей, у которых их не было. Восстановлены "custom
bikes" — и в `<title>` (буквально старый WordPress-заголовок "Rent Motorbike
and Scooter in Bali | Custom Motorbikes", без "Bike Bali Rent —"), и в шапке
рядом с логотипом. WhatsApp/Telegram-ссылки в футере/About теперь открывают
чат с текстом "Hello! 🛵 I want to rent a motorbike." (по-локали) — по
последней правке Дмитрия сообщение **не помечает источник "с сайта"**
(осознанный компромисс, озвучен и принят). Все данные (DNS, DB-миграции
25/25b/26) применены и на dev, и на проде напрямую через SSH/`psql`, никаких
редеплоев БД не требовалось. Подробности — раздел «Сессия 2026-08-01» в п.4
ниже.)

_Предыдущее обновление: 2026-07-30 (сессия: **DNS cutover на `bikebalirent.com`
ЗАВЕРШЁН** — сайт живёт на боевом домене, WordPress больше не в цепочке
для новых запросов. NS делегирован на Cloudflare, A+AAAA переключены на
Contabo, отдельные Let's Encrypt сертификаты выпущены и на apex, и на
`www`. `SITE_ENV=production` включён — индексация открыта
(`robots.txt: Allow: /`), `sitemap.xml` отдаёт 840 реальных URL на 8
языках и принят Google Search Console. По пути найдено и исправлено
несколько реальных багов (протухшая AAAA-запись ещё на Hostinger,
несработавший ACME-запрос из-за неё же, забытая `NEXT_PUBLIC_SITE_URL`
со стейджинга ломала sitemap, `www` без своего сертификата) — подробности
и важный нюанс про недоверенную сеть песочницы агента для этого домена —
раздел «Сессия 2026-07-30» в п.4 ниже. Плюс отдельно: исправлен неверный
Telegram-хендл сайта (`@rents_manager` → `@Bali_rent_main`, коммит
`592da3a`) и добавлен Google Ads conversion tag `AW-17065885486`
(коммит `b365253`, подтверждён рабочим через Tag Assistant).
**Открытый вопрос на конец сессии: WordPress-хостинг на Hostinger ещё
не отключён** — решение когда именно отключать (сразу или подождать
переиндексацию) осталось за Дмитрием, не принято.)
...
**Кэш — половина сделана, половина осталась.** Выяснилось, что нужны ДВЕ вещи,
и это неочевидно:

1. ✅ `Cache-Control` на объектах — проставлено `public, max-age=86400,
   s-maxage=604800` на все 1722 объекта (сутки в браузере, неделя на CDN).
   Проверено по равномерной выборке 41 объекта, пропусков нет. Делалось ДВУМЯ
   проходами по типам файлов (`*.webp` → `image/webp`, `*.mp4` → `video/mp4`) —
   одним проходом с `--content-type image/webp` 35 видео получили бы тип
   картинки и сломались. Команды заливки в `DEPLOY_RUNBOOK.md` обновлены:
   в них добавлен `--cache-control`, иначе новые файлы приедут без заголовка.
2. ❌ **Cache Rule в Cloudflare — НЕ сделано, действие в дашборде.** Одного
   заголовка недостаточно: для R2-домена Cloudflare сам кэшировать не
   начинает, `cf-cache-status` остаётся `DYNAMIC` даже когда `Cache-Control`
   отдаётся корректно (проверено на нескольких объектах, по 2-3 запроса).
   Нужно правило `Hostname equals cdn.bikebalirent.com` → Eligible for cache,
   Edge TTL «use cache-control header». Точные шаги — в `DEPLOY_RUNBOOK.md`,
   раздел «Кэш на cdn.bikebalirent.com».

⚠️ После включения правила: при замене фото по тому же пути обязателен
**Purge Cache**, иначе на узлах CDN до недели будет отдаваться старая картинка.
