...
_Обновлено: 2026-08-26 (сессия: **редизайн мобильного хедера — компактный
языковой дропдаун + постоянный CTA на всех мобильных ширинах**, коммиты
`db72b0a`/`3541386`).

Мобильный хедер: полный список 8 языков (`EN · RU · DE · FR · ES · IT · JA · AR`),
съедавший почти всю ширину `.hdr-in` и не оставлявший места ни под что, кроме
бургера, заменён на компактный `LanguageDropdown` (новый файл, код языка +
inline SVG-глобус + шеврон, дропдаун со всеми локалями по клику) — освободившееся
место отдано под постоянно видимый CTA `dict.home.cta_btn` («Browse bikes» —
ключ уже существовал и был переведён на все 8 языков). Десктоп (`>=860px`) не
тронут — там по-прежнему полный список языков + `nav-desktop`, это не было
предметом жалобы. RTL — только через `dir` на `<html>`, без единой строчки
хардкода под конкретный language-код; подтверждено скриншотами 360/375/414px.
Три черновых варианта (A/B/C) реализованы и просмотрены на dev перед выбором
(Дмитрий выбрал гибрид B/C), затем удалены из репозитория целиком — в финальном
диффе нет мёртвого кода. Попутно пойман и починен CSS-баг каскада (`.lang` было
определено дважды в разных секциях `globals.css`, mobile-override проигрывал
более позднему безусловному правилу той же специфичности). **Инцидент по
пути**: локальная `main` в начале сессии оказалась посторонним
turborepo-скаффолдом (`apps/web`/`apps/api`/`packages/*`), разошедшимся с
`origin/main` на 122 коммита сразу после первого коммита — рабочая директория
переклонирована с нуля с `origin/main`; в `CLAUDE.md` §6 добавлены два
постоянных правила (проверка окружения в начале каждой сессии + запрет
scaffolding-команд внутри рабочей директории проекта). Полные детали — раздел
«Сессия 2026-08-26» в п.4 ниже.

_Предыдущее обновление: 2026-08-20 (сессия: **breadcrumb на статьях блога +
sitemap.xml/hreflang для блога**, коммиты `62a84c9`/`1c90033`). Breadcrumb
(разметка карточки товара была инлайн-JSX — вынесена в общий переиспользуемый
`Breadcrumb.jsx`, теперь на карточке товара и на статье блога один компонент)
+ `sitemap.js` теперь включает все 112 blog URL (14 статей × 8 языков, ранее
блог там не появлялся вообще) с реальным `lastmod` (`GREATEST` по двум
таблицам) и полным hreflang-покрытием через новый `blogHreflangAlternates()`
(per-locale slug статьи не даёт переиспользовать общий
`hreflangAlternates()` напрямую). RTL и мобильная раскладка (375px, перенос
4-уровневого breadcrumb без горизонтального скролла) проверены не на глаз, а
скриптом/`getComputedStyle`. Подробности — раздел «Сессия 2026-08-20» в п.4
ниже.)
...
### Сессия 2026-08-27/28 — диагностика mobile LCP + next/image для hero-фона + Core Web Vitals мониторинг (ЗАВЕРШЕНО, коммиты `ade6326`→`540c335` (revert)→`3e17d12`; миграция `050_seo_performance_snapshots.sql` применена на проде)
...
API. `.env` с ключом лежит только на VPS
(`/root/mdb-seo-monitoring/.env`, `chmod 600`), не в git ни в одном
репозитории. **Если/когда переезжаем с текущего Contabo VPS (`169.58.60.244`)
на другой сервер — если у ключа выставлено IP-restriction на этот адрес,
её нужно обновить на новый IP, иначе мониторинг молча перестанет
работать (PSI вернёт 403, скрипт залогирует ошибку и пропустит URL, но
никто не заметит, если не проверять логи cron).**
