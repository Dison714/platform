...
⚠️ **Кэш Cloudflare НЕ включён** — `cf-cache-status: DYNAMIC` на повторных
запросах, заголовка `cache-control` нет. У объектов в R2 не проставлен
`Cache-Control`, поэтому Cloudflare считает ответы некэшируемыми. Домен тут ни
при чём. Команда массового проставления — в `DEPLOY_RUNBOOK.md`. Решение
отложено: операция перезаписывает метаданные ~1722 объектов, и после каждой
замены фото понадобится Purge Cache.

Мелочь на потом: у Custom Domain стоит **Minimum TLS 1.0** — устаревшая версия.
Для публичных картинок некритично, но поднять до 1.2 стоит (один клик в том же
разделе Custom Domains).
