...
категорий/`display_order` на prod, все локали `200`, логи чистые. Полные
детали — раздел «Сессия 2026-08-19 (продолжение 2)» в п.4 ниже.
...
- **Блог — категории deposit-safety и legal ГОТОВЫ и задеплоены на прод**
  (deposit-safety — сессия 2026-08-11/12; legal — сессия 2026-08-19;
  структура страницы списка — сессия 2026-08-19 продолжение 2, подробности
  — разделы ниже): 14 статей × 8 языков (en/ru/de/fr/es/it/ja/ar),
...
### Сессия 2026-08-19 — чанк Blog: категория legal end-to-end (аудит → контент 8 языков → прод) + баг-фикс markdown-в-excerpt (ЗАВЕРШЕНО, запушено в main `965009c`/`910f752`/`6d6b401`; редеплой Coolify не потребовался — см. поправку ниже)
...
### Сессия 2026-08-19 (продолжение 2) — структура страницы списка блога `/{locale}/blog` (pillar+cluster секции по категориям) + прод-инцидент после редеплоя (ЗАВЕРШЕНО, запушено в main `2535488`, задеплоено и синхронизировано на prod)

**Задача 0 — аудит.** Компонент `[locale]/blog/page.js` был плоским `<ul>`
без карточек/сетки (комментарий в коде прямо называл его каркасом).
Категории — отдельная таблица `article_categories`, не строковое поле;
`display_order` и `articles.is_pillar` уже существовали в схеме
(`047_blog.sql`). UI-строк для страницы блога в locale-словарях не было
(только `nav.blog`). Высота хэдера — `58px` (`.hdr-in`, не 70px как в
референс-макете Дмитрия). Готовый адаптивный паттерн сетки (`.grid`,
2/3/4 колонки на 640/960px) уже используется на `/bikes` и главной —
переиспользован для кластерной сетки блога. RTL — logical properties
(`*-inline-start/end`) уже установленный паттерн сайта, без Tailwind
(его на сайте нет вообще — референс-макет предполагал Tailwind по ошибке,
поймано на этом же шаге).

Живой SELECT (не грепом по сиду) вскрыл расхождение с первоначальным
аудитом: агент, грепавший `048_blog_seed.sql`, нашёл только 5 статей
...
дамп→dry-run→`COMMIT`→независимый `SELECT`.

**Задача 1 — порядок категорий.** `article_categories.display_order` уже
существовал (сид проставил `deposit-safety=1, legal=2`) — свап на
`legal=1, deposit-safety=2` (то, что задумывалось изначально), чистый
`UPDATE`, без `ALTER TABLE`.

**Задача 2 — UI-строки и метаданные категорий, 8 языков.** Найдена дыра:
`article_category_translations.name` был переведён только на `en`,
`description` — `NULL` везде (для обеих категорий). Архитектурное решение:
`name`/`description` — данные категории (Config First, БД,
`/internal/blog`-редактируемо), UI-хром (бейдж «Start here», подпись
«Overview · {n} articles inside») — `blog.*` в locale JSON. Терминология
сверена с реальным опубликованным контентом, а не придумана: es — `fianza`
(не `depósito`), it — `cauzione` (не `deposito`), совпадает с заголовками
живых статей. Отдельная проверка по запросу Дмитрия: арабский `التأمين`
(изначальный кандидат для «депозита» в названии категории) конфликтует со
словом «страхование» — та же лексема используется в соседней статье про
страховку (`bike-insurance-whats-covered-and-whats-not`, 8 вхождений).
Название категории на `ar` заменено на `الوديعة` (однозначно, без
пересечения) — контент уже опубликованных статей (27+8 вхождений
`التأمين`) не тронут, тот же принцип, что и с архивными md-файлами.
Плюрализация (`_one`/`_other`) — подтверждено (агентом), что это
site-wide ограничение (ручной тернарник в `Calculator.jsx:125`, ни одной
i18n-библиотеки/`Intl.PluralRules` в проекте нет) — `_few`/`_two` не
заведены, мёртвая конфигурация без чтения в коде хуже, чем её отсутствие.

**Задача 3 — компонент.** Данные группируются по категории на фронте
(пилар отдельно от кластеров), сортировка категорий и статей — из БД, не
хардкод (готово к 3-й/4-й/5-й категории без правки компонента). Новый
клиентский компонент `frontend/src/app/components/BlogCategoryTabs.jsx`
для sticky-таб-бара — **находка**: обычный `<a href="#slug">` не скроллит
к секции в этом Next.js App Router (хэш меняется, скролл — нет),
воспроизведено многократно, включая через `element.click()` (не только
синтетический клик тестового окружения) — собственный референс-макет
Дмитрия уже это предвидел (`preventDefault`+`scrollIntoView` в JS, не
расчёт на нативное поведение). RTL проверен не визуально, а через
`getComputedStyle` — `border-inline-start` пилар-карточки реально
резолвится в `border-right` при `dir=rtl` (4px, красный). По пути
починен смежный баг: `scrollIntoView({behavior:'smooth'})` в JS
игнорирует `prefers-reduced-motion` (в отличие от CSS-версии) — добавлена
проверка `matchMedia`.

**Порядок кластерных статей — второй раунд ревью Дмитрия.** Скриншоты
показали обратный порядок (14→13→12→11→10 вместо 10→11→12→13→14).
Причина: `ORDER BY published_at DESC` — оба поля-кандидата
(`published_at`, `created_at`) одинаковы у всех статей категории внутри
одной bulk-операции (единый `INSERT` сида и единый `UPDATE` публикации),
подтверждено вживую, что предложенный Дмитрием быстрый фикс
(`created_at ASC`) даёт тот же неверный порядок — гипотеза проверена, а
не применена вслепую. Источник истины — авторская нумерация исходников
`Blog/blog-NN-*.md`. Новая колонка `articles.display_order`
(миграция `049_article_display_order.sql`, значения — прямо в файле
миграции, не отдельным ad-hoc — воспроизводимость с нуля подтверждена)
задаёт порядок кластеров; `ORDER BY` в `blog.js` — `ac.display_order ASC,
a.is_pillar DESC, a.display_order ASC, a.published_at DESC NULLS LAST`.

**Задача 4 — деплой.** `git push` (коммит `2535488`, 13 файлов:
`backend/src/routes/blog.js`, `[locale]/blog/page.js`,
`BlogCategoryTabs.jsx` (новый), `globals.css`, 8 locale-словарей,
`049_article_display_order.sql` (новый)) — подтверждён Дмитрием отдельно
от предыдущих прод-правок дня. Untracked-мусор в корне репозитория
(`.DS_Store`, `00_full_schema.sql`…`24_family_specs_sort.sql`,
`Google Ads Photos/`, `.claude/` и т.д.) — не тронут, staged только
конкретные 12 файлов задачи.

**Прод-инцидент.** Дмитрий выполнил редеплой Coolify до применения
миграции `049` на prod — `/api/blog/posts` начал падать 500
(`articles.display_order` не существовал в prod-схеме), проявилось и на
скриншоте `/ru/bikes` (тот же digest, что и `/ru/blog` в логах — общий
серверный error boundary Next.js). Восстановлено штатным циклом: `pg_dump`
прод-бэкап (`prod_backup_2026-08-19_pre_display_order.dump`, сохранён и на
сервере, и локально в scratchpad) → dry-run `049` с `ROLLBACK` → явное
подтверждение Дмитрия → `COMMIT` → независимый `SELECT`. Заодно
досинхронизирован остаток Задачи 4, который ещё не уехал на prod: 16
строк `article_category_translations` (name+description, 8 языков ×
2 категории) и свап `display_order` категорий — до этого `ru/blog` на
проде показывал сырые slug'и (`deposit-safety`/`legal`) вместо названий.
После обеих правок — все проверенные локали (`en/ru/ar/de`) на `/blog`
и `/blog/[slug]` отдают `200`, 0 ошибок в логах обоих контейнеров за
5 минут, живые заголовки категорий на `ru`/`ar` корректны.

**Верификация.** Headless Chrome CLI (`--window-size=375,812
--screenshot`) дал ложный алярм на мобильном скриншоте прод-страницы —
текст выглядел обрезанным справа; перепроверено через настоящую
мобильную эмуляцию (`resize_window` в Browser pane, Android UA + touch) —
рендер корректный, бага нет. Урок: `--window-size` в headless CLI не
эквивалентен мобильной эмуляции, для мобильной проверки использовать
только полноценный device-emulation инструмент. Живой клик по табу с
плавным скроллом в реальном браузере — **не проверен** (песочница
Browser pane держит `document.hidden: true` даже у «активной» вкладки,
что известно ломает RAF-таймингом smooth-scroll в headless-окружениях;
сам механизм доказан напрямую через DOM: `scroll-margin-top` считается
верно, `884px = offsetTop 1000 − 116`, обработчик клика подключён и
вызывается корректно) — стоит в списке для проверки Дмитрием на проде в
обычном браузере.

**Файлы:** см. Задачу 4 выше. БД: `articles.display_order` (новая
колонка, 049), `article_categories.display_order` (свап),
`article_category_translations` (16 upsert). Prod-бэкап
`prod_backup_2026-08-19_pre_display_order.dump`.
