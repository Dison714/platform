...
_Обновлено: 2026-07-30 (сессия: **DNS cutover на `bikebalirent.com`
...
(коммит `b365253`, подтверждён рабочим через Tag Assistant). Email
(MX/SPF/DKIM/DMARC) сознательно не тронут, остаётся на Hostinger.
...
_Предыдущее обновление: 2026-07-29 (сессия: R2-ресинк фото после пересборки прошлой
сессии + фикс content-type видео, доступ на прод переведён на прямой SSH
(см. CLAUDE.md §6), заполнены цены 22 продуктам без полного 30-дневного
тарифа, breadcrumb на карточке товара по пути фильтра — задеплоен, 7
переименований карточек, заменено OG-превью фото. Известный неисправленный
баг — видео без звука (`-an` в ffmpeg-пайплайне), осознанно отложен в
отдельный чат. Подробности — раздел «Сессия 2026-07-29» в п.4 ниже.)
