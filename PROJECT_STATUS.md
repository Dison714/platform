...
_Обновлено: 2026-09-10 (сессия: **9 SEO-районных страниц
...
- **Блог — категории deposit-safety и legal ГОТОВЫ и задеплоены на прод**
  ...
  список. **Остаётся:** 3 категории (bike-models/routes/digital-nomads,
  15 статей) — на dev и prod всё ещё draft-заглушки (en title/slug из
  сида), нужен тот же цикл контент→перевод→ссылки→publish→sync; компонент
  страницы списка их подхватит автоматически, как только появятся
  published-статьи (рендерится из данных, категории не хардкожены).
