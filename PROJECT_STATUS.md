...
### ⚠️ НАЙДЕНО: админка Coolify торчит в интернет по голому HTTP (НЕ ИСПРАВЛЕНО)
...
**Как чинить (действия Дмитрия — это его дашборды и системные настройки):**
...
2. Coolify → Settings → Instance's Domain (FQDN) → `https://coolify.bikebalirent.com`.
   Coolify выпустит Let's Encrypt сертификат своим Traefik автоматически.
3. Проверить, что `https://coolify.bikebalirent.com` открывается с валидным
   сертификатом, и **только после этого** закрывать 8000.
4. Закрыть 8000/8080 снаружи. ⚠️ Осторожно с ufw: включение с дефолтным
   `deny incoming` без явного разрешения SSH **отрежет доступ к серверу**.
   Порядок обязателен — сначала `allow 22`, потом enable:

```bash
ufw allow 22/tcp && ufw allow 80/tcp && ufw allow 443/tcp
ufw --force enable          # только после allow 22, иначе потеря SSH
ufw status verbose
```

Claude это не делает сам: изменение системных/security-настроек прода —
не то, что должно происходить молча, а ошибка в ufw стоит потери доступа
к серверу.
