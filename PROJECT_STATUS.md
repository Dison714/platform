...
_Обновлено: 2026-08-28 (сессия: **диагностика mobile LCP → next/image для
hero-фона → Core Web Vitals мониторинг**, коммиты
`ade6326`→`540c335`(revert)→`3e17d12`→`c7ad972`→`389af67`).

Read-only диагностика LCP=8.0с на mobile (PSI) по 6 блокам → первая попытка
фикса (`ade6326`: preload hero-фона + eager-карточки) дала **регрессию**
(LCP 8.0с→11.3с) → откат (`540c335`) → root cause найден через локальный
Lighthouse CLI (методология PSI воспроизведена локально): реальный
LCP-элемент — текстовый `<h1>`, не картинка; `fetchpriority=high` на
646-килобайтном CSS-фоне отбирал полосу у CSS, гейтящего рендер настоящего
LCP-элемента. Финальный фикс (`3e17d12`): CSS `background-image` заменён на
`next/image fill priority` (плюс добавлена ранее отсутствовавшая
зависимость `sharp` — без неё оптимизатор `next/image` не работает на
self-hosted, blocker подтверждён и снят). Локальный Lighthouse и реальный
PSI после фикса всё ещё расходятся на порядок (~2.3с vs ~8с) — причина не
найдена, открытый вопрос. Настроен ежедневный мониторинг Core Web Vitals
(`Dison714/mdb-seo-monitoring`, отдельный приватный репозиторий — Coolify
`watch_paths` пуст, редеплой платформы push'ем в него не триггерится) —
таблица `seo_performance_snapshots` (миграция 050), крон на VPS 03:00 UTC.
Полные детали — раздел «Сессия 2026-08-27/28» в п.4 ниже.

_Предыдущее обновление: 2026-08-26 (сессия: **редизайн мобильного хедера —
компактный языковой дропдаун + постоянный CTA на всех мобильных ширинах**,
коммиты `db72b0a`/`3541386`). Мобильный хедер: полный список 8 языков
заменён на компактный `LanguageDropdown` (код языка + inline SVG-глобус +
шеврон), освободившееся место отдано под постоянно видимый CTA. Десктоп не
тронут. **Инцидент по пути**: локальная `main` оказалась посторонним
turborepo-скаффолдом, разошедшимся с `origin/main` на 122 коммита —
переклонировано с нуля; в `CLAUDE.md` §6 добавлены проверка окружения в
начале сессии + запрет scaffolding-команд. Подробности — раздел «Сессия
2026-08-26» в п.4 ниже.)
