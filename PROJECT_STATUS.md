...
### Сессия 2026-09-14/15 — перелинковка блога + аудит http/hreflang + self-referencing canonical для модель-хабов (ЗАВЕРШЕНО п.1/3/4 + п.2 шаги 1-2; коммиты `36c3c59`→`91b76fc` в `platform`, локальные, редеплой Coolify не запускался; DB-контент (55 вставок ссылок в статьи + FAQ) применён напрямую и на прод, и на dev)
...
Итог сессии: коммиты `36c3c59`/`91b76fc` — локальные, не запушены и не
задеплоены (как обычно, редеплой frontend через Coolify — за Дмитрием после
отдельного подтверждения на push).
