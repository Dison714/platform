...
_Обновлено: 2026-08-19, продолжение 2 (сессия: **страница списка блога
`/{locale}/blog` — секции по категориям (pillar+cluster) вместо плоского
списка, плюс прод-инцидент после редеплоя и его фикс**, коммит `2535488`).

Плоский список статей на `/{locale}/blog` заменён на секции по категориям:
пилар-карточка (бейдж, акцентная граница) сверху, кластерная сетка под ней,
sticky-табы-якоря вверху страницы со счётчиком статей на категорию —
рендерится из данных (`article_categories.display_order`), не хардкод
списка категорий. Новая колонка `articles.display_order` (миграция `049`)
задаёт читательский порядок кластерных статей внутри категории — ни
`published_at`, ни `created_at` для этого не годились (оба одинаковые
внутри bulk-операций сессии, проверено вживую перед тем как поверить
предположению). Источник значений — авторская нумерация `Blog/blog-NN-*.md`.
`article_categories.name`/`description` переведены на все 8 языков (были
только `en`). RTL проверен не на глаз, а через `getComputedStyle`
(`border-inline-start` реально резолвится в `border-right` при `dir=rtl`).
**Прод-инцидент после редеплоя** (не связан с кодом самой задачи, а с
порядком операций): Дмитрий выполнил редеплой Coolify раньше, чем миграция
`049` была применена на prod — `articles.display_order` не существовал,
`/api/blog/posts` отдавал 500 (проявилось на `/bikes` тоже — общий
серверный экшен, тот же digest). Починено штатным циклом (`pg_dump`-бэкап →
dry-run с `ROLLBACK` → подтверждение → `COMMIT` → независимая проверка
живым `curl`) — применена миграция `049` и синхронизированы переводы
категорий/`display_order` на prod, все локали `200`, логи чистые. Живой
клик по табу на проде — **подтверждён Дмитрием лично**, работает. Полные
детали — раздел «Сессия 2026-08-19 — чанк Blog: категория legal…» в п.4
ниже (продолжение того же дня, во второй половине раздела).

_Предыдущее обновление: 2026-08-19 (сессия: **чанк Blog: категория legal
end-to-end (аудит → контент 8 языков → прод) + баг-фикс markdown-в-excerpt**,
коммиты `965009c`/`910f752`/`6d6b401`). 6 статей категории `legal`
переведены и опубликованы на всех 8 языках (48 `article_translations`),
цикл дамп→dry-run→`COMMIT`→независимый `SELECT` закреплён как обязательный
стандарт для всех будущих правок прод-данных. Найден и исправлен баг:
markdown-синтаксис утекал в `excerpt` (страница списка блога рендерит его
как plain text) — guard `assertCleanExcerpt()` добавлен в оба
`gen_*_blog_sync.mjs`. Подробности — раздел «Сессия 2026-08-19 — чанк Blog:
категория legal…» в п.4 ниже.)
