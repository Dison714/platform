...
**IPv6 — вывод по документации, не эмпирическая проверка.** Docs Contabo про
значение `Any` в Source: «Allows traffic from all IP addresses (IPv4 and
IPv6)», правила применяются на сетевом уровне. У `Block all traffic` Source =
`Any`, ACCEPT-правил для этих портов нет → закрыты и по v6. Прямой фразы
«DROP покрывает оба протокола» в статье нет, так что это вывод.
Эмпирически не подтверждено: IPv6-связности нет ни на машине разработчика, ни
у мобильного оператора Дмитрия (контрольный тест на порт 80 по IPv6-литералу
дал ту же ошибку, что и основной — признак отсутствия IPv6 у клиента, а не
закрытого порта). При необходимости — внешний сканер портов с поддержкой IPv6.

⚠️ Важно для контекста: AAAA публикуются (`bikebalirent.com` и `www` →
`2a02:c207:2345:9293::1`), то есть IPv6-адрес сервера узнаётся одним `dig`.
Если Contabo когда-нибудь окажется IPv4-only — порты станут доступны по v6.
