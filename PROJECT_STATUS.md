...
**Единый коммит подготовлен** на: font-фикс (`layout.js`), чек-лист §3.8
(`CLAUDE.md`), 6 скриптов переводов (`backend/scripts/apply_family_content.mjs`,
`audit_all_translations.mjs`, `audit_category_translations.mjs`,
`audit_family_content.mjs`, `family_content_data.mjs` — уже с переведёнными
h3-заголовками, `gen_family_content_sync.mjs`), редирект slug
(`frontend/next.config.mjs`), обновление этого раздела. Данные трёх таблиц
переводов, title-фикс Kawasaki и rename slug — уже в БД на dev и на проде
(это данные, не файлы; сгенерированные `*_sync.sql` в корне репозитория —
untracked по устоявшейся конвенции, ни один `_sync.sql` в репозитории
никогда не трекался). `MDB_Platform_TZ.docx` — отдельный файл вне
git-репозитория, уже перезаписан версией с чек-листом §3.8 (бэкап
предыдущей версии — `/tmp/MDB_Platform_TZ_backup_before_edit.docx`,
локально). **Push/redeploy — ждёт отдельного явного подтверждения**, не
выполнялся в этой сессии.
